                                           ...
                                           errcode_t status)
{
    ...
    if ((write_cb_para->length > 0) && write_cb_para->value) {
        ...
        // 确保目标缓冲区足够大
        memcpy(g_get_sle_data, write_cb_para->value, write_cb_para->length);
        g_get_sle_data[write_cb_para->length] = '\0';
        // strncpy(g_get_sle_data, write_cb_para->value, write_cb_para->length);
        g_IS_READ_DATA_FLAG = 1;
