...
at_ret_t get_wifi_info(void)
{
    uint16_t key_len = (uint16_t)sizeof(openvalley_wifi_info_t);
    uint16_t real_len = 0;
...
at_ret_t set_wifi_info(const openValley_wifi_args_t *args)
{
    uint16_t key_len = (uint16_t)sizeof(openvalley_wifi_info_t); // 写NV SLE payload的长度
    uint16_t real_len = 0;
    openvalley_wifi_info_t *read_value = NULL;
